...
myApp.post("/createAdd", upload.single("image"), async (req, res) => {
  try {
    const { title, price, image, quantity, description, category } = req.body;
...
// Add To Cart
// myApp.post("/addToCart", async (req, res) => {
//     try {
//         const userId = req.body.userLoggedIn._id;
...
//         const product = await Adds.findById(productId);
//         if (!product) {
//             return res.status(404).json({ msg: "Product not found" });
//         }

//         // Check if the product is already in the cart
//         const existingCartItem = user.Cart.find(item => item._id.toString() === productId.toString());
//         if (existingCartItem) {
...
// Update Quantity
myApp.put("/updateQuantity/:id", async (req, res) => {
  try {
    const id = req.params.id;
    ...
    const product = await Adds.findOneAndUpdate(
      { _id: id },
...
myApp.put("/checkoutQuantityUpdate", async (req, res) => {
  try {
    const cart = req.body;

    const updatePromises = cart.map((product) => {
      return Adds.findById(product._id).then((foundProduct) => {
        if (foundProduct) {
          const newQuantity = foundProduct.quantity - product.quantity;
          return Adds.findByIdAndUpdate(
            product._id,
            { quantity: newQuantity },
            { new: true }
          );
...
myApp.post("/addToFav", async (req, res) => {
  try {
    ...
    const productInFav = await Adds.findById(productId);
    if (!productInFav) {
      return res.status(404).json({ msg: "Product not found" });
    }

    // Check if the product is already in the Fav array
    const existingCartItem = user.Fav.find(
      (item) => item._id.toString() === productId.toString()
    );

    if (existingCartItem) {
      // Remove item from Fav array
      user.Fav = user.Fav.filter(
        (item) => item._id.toString() !== productId.toString()
      );
      res.json({ msg: "Product removed from Favorites" });
    } else {
      // Add item to Fav array
      user.Fav.push({ _id: product._id });
...
myApp.get("/getUserFavorites/:userId", async (req, res) => {
  try {
