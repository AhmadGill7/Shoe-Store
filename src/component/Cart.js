...
export default function Cart({ userLoggedIn }) {
  ...
  // Quantity Reduction
  const checkoutQuantityUpdate = async (cart) => {
    try {
      if (userLoggedIn) {
        const response = await axios.put(`/checkoutQuantityUpdate`, cart);
  ...
  const handleCheckout = async () => {
    if (userLoggedIn) {
      const stripe = await loadStripe(REACT_APP_STRIPE_PRIVATE_KEY);
      const response = await axios.post(`/create-checkout-session`, cart);

      const result = stripe.redirectToCheckout({
        sessionId: response.data,
      });
      checkoutQuantityUpdate(cart);
